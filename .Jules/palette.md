...
## 2026-08-16 - Stateful URLs for Tabbed Interfaces (Deep Linking)
**Learning:** Single-page applications with tabbed interfaces (like solver selections) often trap users by keeping navigation state purely in memory. This prevents users from bookmarking a specific tab, refreshing the page without losing their place, or sharing a direct link to a particular tool with a colleague. This breaks expected browser behavior and causes frustration.
**Action:** Always map primary UI states (like active tabs) to the URL using hash fragments (e.g., `#stochastic`) or query parameters. Listen to the `hashchange` event to support back/forward browser navigation, and ensure the initial load correctly reads the fragment to restore the state, providing a robust, shareable UX.
