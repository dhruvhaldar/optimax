...
## 2026-03-12 - Bypass of Chunked Encoding Restriction via Multiple Headers
**Vulnerability:** The `limit_request_size` middleware checked for 'chunked' encoding using `request.headers.get("transfer-encoding")`, which only returns the first header value. An attacker could send multiple Transfer-Encoding headers (e.g., `Transfer-Encoding: gzip` followed by `Transfer-Encoding: chunked`), causing the middleware to only see 'gzip', bypass the block, and potentially trigger a Denial of Service via a massive chunked payload.
**Learning:** In FastAPI/Starlette, `request.headers.get()` and standard dictionary access (`request.headers[...]`) only return the *first* value for a given header key. For security-critical checks on headers that can be specified multiple times (like `Transfer-Encoding`), all values must be inspected.
**Prevention:** Changed the check to use `request.headers.getlist("transfer-encoding")` and iterating through all values with `any()` to ensure 'chunked' is detected even if it appears in a subsequent header.
