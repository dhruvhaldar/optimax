    ...
    <form aria-labelledby="solver-title" className="glass-panel p-6" onKeyDown={handleKeyDown} onSubmit={e => { e.preventDefault(); if (!loading) solveLP(); }}>
      ...
      <div className="mb-4">
        ...
        <input
          ...
          aria-describedby={invalidField === 'c' ? "lp-error" : undefined}
      ...
      <div className="mb-4">
        ...
        <textarea
          ...
          aria-describedby={invalidField === 'A' ? "lp-error" : undefined}
      ...
      <div className="mb-4">
        ...
        <input
          ...
          aria-describedby={invalidField === 'b' ? "lp-error" : undefined}
      ...
      <fieldset className="mb-6 disabled:opacity-50 disabled:cursor-not-allowed" disabled={loading}>
        ...
        <div className="flex items-center space-x-6">
          ...
          <label htmlFor="lp-minimize" className="flex items-center space-x-3 cursor-pointer">
            <input
              id="lp-minimize"
              type="radio"
              name="lp-objective-direction"
      ...
      >
        {loading ? (
          ...
          <>
            Solve LP <kbd aria-hidden="true" className="text-xs opacity-80 ml-2 font-mono hidden sm:inline px-1.5 py-0.5 bg-white/10 border border-white/20 rounded-md shadow-sm">{shortcutSymbol}</kbd>
          </>
        )}
      </button>

      {error && (
        <div id="lp-error" role="alert" aria-live="assertive" className="mt-4 p-4 bg-red-500/20 border border-red-500/50 rounded-lg text-red-200">
          Error: {error}
        </div>
      )}
        ...
        <div className={`mt-8 p-6 bg-black/20 rounded-xl border border-white/5 transition-opacity duration-200 ${loading ? 'opacity-50 pointer-events-none' : ''}`}>
          ...
          <dl className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div className="bg-white/5 p-4 rounded-lg">
              ...
              <dd className="text-lg font-semibold text-white">{result.fun !== null ? result.fun.toFixed(4) : "N/A"}</dd>
            </div>
