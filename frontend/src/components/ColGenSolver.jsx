...
const ColGenSolver = () => {
  ...
  const solveColGen = async () => {
    ...
    try {
      ...
      setError(
        "Invalid input format. Please ensure demands are formatted as a valid JSON matrix (e.g., [[3, 25], [5, 20]]).",
      );
      setLoading(false);
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div className="mb-4">
        <label
          htmlFor="colgen-demands"
          className="block text-sm font-medium text-slate-300 mb-2"
        >
          Demands (Width, Quantity){" "}
          <span className="text-red-400" aria-hidden="true">
            *
          </span>
          :
        ...
        >
          ...
          <dl className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div className="bg-white/5 p-4 rounded-lg">
              <dt className="text-slate-400 block text-sm">
                Rolls Used (LP Relaxation)
              </dt>
              <dd className="text-lg font-semibold text-white">
                {result.objective !== null && result.objective !== undefined
          ...
          <ul className="space-y-2 mb-6">
            {result.patterns.map((pat, idx) => (
              <li
                key={idx}
                className="bg-white/5 p-3 rounded flex flex-col sm:flex-row justify-between items-start sm:items-center border border-white/10 gap-2"
              >
                ...
                <span className="text-slate-400 text-sm whitespace-nowrap shrink-0">
                  x_{idx + 1} = {result.solution[idx]?.toFixed(2)}
                </span>
              </li>
            ))}
          </ul>
