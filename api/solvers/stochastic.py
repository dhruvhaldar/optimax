...
def solve_stochastic(total_land, scenarios):
    ...
    # Optimization: Use strided assignment instead of np.column_stack().ravel() to avoid intermediate array allocation
    ...
    # Optimization: Use strided assignment
    ...
    vals_ub[idx:idx + n_scenarios] = 1.0
    ...
    # Optimization: Use strided assignment
    cols_eq = np.zeros(3 * n_scenarios, dtype=int)
    cols_eq[0::3] = 2
    cols_eq[1::3] = base_indices + 4
    cols_eq[2::3] = base_indices + 5

    vals_eq = np.zeros(3 * n_scenarios)
    vals_eq[0::3] = ylds[:, 2]
    vals_eq[1::3] = -1.0
    vals_eq[2::3] = -1.0
